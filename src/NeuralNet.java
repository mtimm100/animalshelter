...
public class NeuralNet {
	...
	/*
	 * Train the neural network, evaluate the model, print results
	 * Return the percent correctly classified
	 */
	public double evalNeuralNet(){
		try {
			//this.mlp.buildClassifier(trainingSet);
			Evaluation eval = new Evaluation(this.trainingSet);
			if(trainingSet.size() < 10){
				eval.crossValidateModel(mlp, trainingSet, trainingSet.size(), new Random(1));
			}
			else{
				eval.crossValidateModel(mlp, trainingSet, 10, new Random(1));
			}
